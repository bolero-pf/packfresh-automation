...
# gid_numeric imported from shared/shopify_graphql.py
...
from shopify_graphql import shopify_gql as _base_shopify_gql, shopify_metafields_set as _base_metafields_set, gid_numeric

def shopify_gql(query: str, variables=None):
    """Debug/dry-run wrapper around the shared Shopify GraphQL client."""
    ...
    if VIP_DRY_RUN:
        ...
        return {}
    t0 = time.time()
    resp = _base_shopify_gql(query, variables)
    dlog(f"[GQL] ok in {time.time()-t0:.3f}s")
    return resp

def shopify_metafields_set(inputs: Iterable[Dict[str, Any]]) -> None:
    """Convenience wrapper — routes through our debug shopify_gql."""
    mutation = """
