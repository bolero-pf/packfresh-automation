...
def charm_drop_auto_threshold_slab(price) -> float:
    """Maximum dollar drop the slab updater auto-applies without flagging.
    Pinned to one charm_ceil tier — within one charm hop a "drop" is just
    rounding noise, not a real comp move. Slabs live on the in-store hub
    and don't have a Shopify price-drop notification audience, so we don't
    need the strict "all drops review" rule sealed has.
...
def run(*, apply: bool = True, csv_path: str = None) -> list[dict]:
    ...
    for slab in slabs:
        ...
        # Bigger drops still need a human eye — defense against a corrupt
        # comp pulling a $50 card to $5. Slabs live on the in-store hub
        # so there's no Shopify price-drop notification audience to time;
        # that's a sealed-only concern.
