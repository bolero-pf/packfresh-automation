...
- **app.py** — Flask app: tabbed dashboard UI (SKUs + Inventory Flow), /run pipeline trigger, /api/* endpoints
  ...
  - Dead capital also gates on **in-stock age ≥ DEAD_MIN_AGE_DAYS (45)** using the earliest `sku_daily_inventory` snapshot as "in stock since" — brand-new stock with no sales isn't dead, it just arrived (this halved a misleading 519/$139k down to the real stuck capital). Restricted to status='ACTIVE' (drafts can't sell). The KPI "Dead Capital" uses the SAME predicate as the Job A list so they match.
  - Endpoints: `/api/inventory/flow` (combined KPIs + sealed roll-up), `/api/inventory/raw` (raw roll-up), `/api/inventory/raw-aging` (raw dead capital), `/api/inventory/dead` (Job A: aged zero-sales OR slow<0.1/day AND piled>180d — fast sellers w/ deep stock are NOT flagged; returns total + top 300), `/api/inventory/dead.csv` (full worklist export), `/api/inventory/restock` (Job B: ≤30 days left at true rate).
