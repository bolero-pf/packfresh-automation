...
- **app.py** — Flask app: tabbed dashboard UI (SKUs + Inventory Flow), /run pipeline trigger, /api/* endpoints
  ...
  - **Margin honesty**: cost-less catalog sales (`gross_margin` NULL, ~20-25% of catalog rev — older pre-cost-tracking stock) book $0 margin, never full. Margin **%** is computed on COST-KNOWN revenue (proven rate), not all revenue. **Est. total margin** (`_margin_estimate`) covers the FULL window: proven margin + a per-category-rate projection of EVERY other merchandise dollar in the window — pre-COGS-era sales (before 2026-03-23), no-cost stock, and suspect-cost rows alike — so "90 days" of est. margin really spans 90 days (catalog merch base = sku_daily_sales × product_taxonomy full window; raw from raw_cards). `cost_coverage` = proven_costed / total_merch says how much is proven vs estimated (drops on longer windows that reach before the COGS era). Labeled a CONSERVATIVE FLOOR: older stock runs higher-margin than the recent category rate we project at, AND recorded COGS is itself often overstated, so true margin exceeds both proven and estimated. Endpoints: `/api/sales/overview` (KPIs + prior-period deltas + daily series: net/orders/units/in-store/shipped/margin), `/api/sales/buysell` (weekly intake spend by `ingested_at` vs net sales), `/api/sales/mix` (weekly stacked revenue by stream or game). **Margin** = catalog `realized_margin.gross_margin` + raw singles (`sale_price-cost_basis`); margin% is vs merch revenue (realized_margin.revenue + raw sale_price), not order totals. **Raw singles** are categorized straight from `raw_cards` by barcode (game/set/graded) — their revenue is ALREADY in `customer_orders` (barcode line items), so they're a reclassification, never added on top.
