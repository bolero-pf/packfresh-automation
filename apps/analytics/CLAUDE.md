...
- **app.py** — Flask app: tabbed dashboard UI (SKUs + Inventory Flow), /run pipeline trigger, /api/* endpoints
  ...
  - **Break-down decision (`_dead_action`)**: for dead SEALED it's a LIQUIDITY call, judged against COST basis (`inventory_product_cache.unit_cost`), NOT the inflated sealed price that's keeping it dead. Per-unit break value = `sealed_breakdown_cache.best_variant_market` (component market, keyed by tcgplayer_id; analytics reads it, doesn't refresh it). parts≥cost → "break down, free +$X over cost"; parts≥0.8×cost → "break to free cash (small loss OK for velocity)"; else markdown/hold. Comparing parts vs *price* is misleading (almost nothing clears it); vs *cost*, ~21 items recover ~$6k. The list/CSV show cost · sealed · parts so the operator can judge. CAVEAT: unit_cost is only populated for ~42% of dead sealed, recipes for ~38% — items missing either can't get a break recommendation. Non-sealed: vintage/high-value→reprice-hold, <$15→bundle, else markdown.
