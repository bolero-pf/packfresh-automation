...
class ShopifyClient:
    ...
    def iter_products_pages(self, batch_size: int = 100):
        ...
        while has_next:
            ...
            for edge in edges:
                ...
                is_damaged = ("damaged" in [t.lower() for t in (tags if isinstance(tags, list) else [])]) or "[DAMAGED]" in node.get("title", "").upper()
    ...
    # ─── Inventory REST helpers (used by inventory service) ─────────────────
    ...
    def _rest(self, method: str, path: str, **kwargs) -> dict:
        url = f"https://{self.store}/admin/api/2025-10{path}"
        resp = requests.request(method, url, headers=self._rest_headers(), timeout=30, **kwargs)
        resp.raise_for_status()
        return resp.json()

    def _rest_update_variant_price(self, variant_id: int, price: float) -> None:
        self._rest("PUT", f"/variants/{variant_id}.json", json={
            "variant": {"id": variant_id, "price": f"{price:.2f}"}
        })

    def _rest_set_inventory(self, inventory_item_id: int, location_id: int, qty: int) -> None:
